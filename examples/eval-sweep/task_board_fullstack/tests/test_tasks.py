...
def test_crud_roundtrip(client):
    ...
    created = client.post('/tasks', json={'title': 'buy milk'}, headers=headers)
    assert created.status_code == 201
    item_id = created.json()['id']
    listed = client.get('/tasks', headers=headers)
    assert listed.status_code == 200
    assert any(i['id'] == item_id for i in listed.json())
    updated = client.patch(f'/tasks/{item_id}', json={'completed': True}, headers=headers)
    assert updated.status_code == 200
    assert updated.json()['completed'] is True
    deleted = client.delete(f'/tasks/{item_id}', headers=headers)
    assert deleted.status_code == 204

def test_crud_404_for_missing(client):
    _register(client)
    headers = _headers(_token(client))
    assert client.get('/tasks/9999', headers=headers).status_code == 404
