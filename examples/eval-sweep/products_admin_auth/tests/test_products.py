...
def test_crud_roundtrip(client):
    ...
    created = client.post('/products', json={'title': 'buy milk'}, headers=headers)
    assert created.status_code == 201
    item_id = created.json()['id']
    listed = client.get('/products', headers=headers)
    assert listed.status_code == 200
    assert any(i['id'] == item_id for i in listed.json())
    updated = client.patch(f'/products/{item_id}', json={'completed': True}, headers=headers)
    assert updated.status_code == 200
    assert updated.json()['completed'] is True
    deleted = client.delete(f'/products/{item_id}', headers=headers)
